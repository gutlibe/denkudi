...
function CandidatePhoto({
    ...
    return (
        <div className="relative flex h-24 w-24 shrink-0 items-center justify-center overflow-hidden rounded-2xl transition-all duration-200">
            ...
            {isSelected && (
                <div className="absolute right-1.5 bottom-1.5 flex h-6 w-6 items-center justify-center rounded-full bg-primary shadow-md">
...
export function VoteFlowDesktop({ election, open, onClose, onVoted }: Props) {
    ...
    return (
        <Sheet open={open} onOpenChange={handleOpenChange}>
            <SheetContent
                side="right"
                className="flex w-full flex-col gap-0 border-0 bg-gradient-to-b from-background to-card p-0 sm:max-w-xl"
                        ...
                        <div className="relative -mt-4 flex flex-1 flex-col overflow-hidden rounded-t-3xl border-x border-t border-border/30 bg-muted/60 shadow-lg shadow-black/5 dark:border-border/10 dark:bg-card dark:shadow-none">
                                ...
                                {currentPosition.candidates.map((candidate) => {
                                    ...
                                    return (
                                        <button
                                            ...
                                            className={`flex w-full items-center gap-4 rounded-2xl border p-4 text-left transition-all duration-150 ${isSelected ? 'border-primary bg-primary/5 shadow-sm ring-1 shadow-primary/5 ring-primary/30' : 'border-border/50 hover:border-border hover:bg-muted/30'}`}
                                        >
                                            <CandidatePhoto
                                                candidate={candidate}
                                                isSelected={isSelected}
                            ...
                            <div className="shrink-0 border-t border-border/40 bg-muted/60 px-6 py-4 dark:bg-card">
                                <div className="mb-3 flex items-center justify-between text-xs text-muted-foreground">
                                    <span>
                                        {positionsCompleted} of{' '}
                                        {positions.length} positions selected
                                    </span>
                                    <span>
