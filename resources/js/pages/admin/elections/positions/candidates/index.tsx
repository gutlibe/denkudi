...
export default function CandidatesIndex({ election, position }: Props) {
    ...
    const [viewManifesto, setViewManifesto] = useState<{
        name: string;
        text: string;
    } | null>(null);
    ...
    const submit = (e: React.FormEvent) => {
        ...
        if (dialog.edit) {
            ...
            router.post(
                `${base}/${dialog.edit.id}`,
                data as unknown as Record<string, string | File>,
                {
                    forceFormData: true,
                    onSuccess: () => {
    ...
    return (
        <>
                    ...
                    <div>
                        ...
                        <p className="mt-1 text-muted-foreground">
                            {position.max_selections === 1
                                ? 'Single choice'
                                : `Up to ${position.max_selections}`}{' '}
                            &middot; {position.candidates.length} candidate
            ...
            <Dialog
                open={dialog.open}
                onOpenChange={(o) =>
                    !o && setDialog({ open: false, edit: null })
                }
            >
                <DialogContent>
                    <DialogHeader>
                        <DialogTitle>
                            {dialog.edit ? 'Edit Candidate' : 'Add Candidate'}
                        </DialogTitle>
                        <DialogDescription>
                            {dialog.edit
                                ? 'Update candidate details.'
                                : `Add a candidate for ${position.title}.`}
                        </DialogDescription>
                    </DialogHeader>
                    <form
                        onSubmit={submit}
                    ...
                    >
                        ...
                        <div className="grid gap-2">
                            <Label htmlFor="cand-manifesto">
                                Manifesto ({form.data.manifesto.length}/5000)
                            </Label>
                            <Textarea
                                id="cand-manifesto"
                                rows={4}
                                maxLength={5000}
                                value={form.data.manifesto}
                                onChange={(e) =>
                                    form.setData('manifesto', e.target.value)
                                }
                    ...
                    <DialogFooter>
                        ...
                        <Button
                            ...
                            onClick={() => {
                                if (deleteId) {
                                    router.delete(
                                        `/admin/elections/${election.id}/candidates/${deleteId}`,
                                    );
                                }
            ...
            <Dialog
                open={viewManifesto !== null}
                onOpenChange={() => setViewManifesto(null)}
            >
                <DialogContent>
                    <DialogHeader>
                        <DialogTitle>{viewManifesto?.name}</DialogTitle>
                        <DialogDescription>Manifesto</DialogDescription>
                    </DialogHeader>
                    <ScrollArea className="max-h-80">
                        <div className="py-2 pr-4 text-sm leading-relaxed whitespace-pre-wrap">
                            {viewManifesto?.text}
                        </div>
                    </ScrollArea>
                    <DialogFooter>
